/****************************************************************************
 * arch/arm/src/hpmicro/hpm6750/hpm_ehci.c
 *
...
#define HPM_EHCI_NRHPORT                 1     /* There is only a single root hub port */

/* Configurable number of Queue Head (QH) structures.  The default is one per
 * Root hub port plus one for EP0.
 */

#ifndef CONFIG_HPM_EHCI_NQHS
#  define CONFIG_HPM_EHCI_NQHS (HPM_EHCI_NRHPORT + 1)
#endif
...
#ifndef CONFIG_HPM_EHCI_NQTDS
#  define CONFIG_HPM_EHCI_NQTDS (HPM_EHCI_NRHPORT + 3)
#endif

/* Buffers must be aligned to the cache line size */

#define DCACHE_LINEMASK (HPM_L1C_CACHELINE_SIZE - 1)
...
#define HPM_EHCI_BUFSIZE \
  ((CONFIG_HPM_EHCI_BUFSIZE + DCACHE_LINEMASK) & ~DCACHE_LINEMASK)

/* Debug options */

#ifndef CONFIG_DEBUG_USB_INFO
#  undef CONFIG_HPM_EHCI_REGDEBUG
#endif
 ...
 *
 * However, for the case of the HPM EHCI, we know apriori that locations
...
struct hpm_qh_s
{
  ...
  struct hpm_epinfo_s *epinfo; /* Endpoint used for the transfer */
...
struct hpm_list_s
{
  struct hpm_list_s *flink;    /* Link to next entry in the list
...
typedef int (*foreach_qh_t)(struct hpm_qh_s *qh, uint32_t **bp,
                            void *arg);
typedef int (*foreach_qtd_t)(struct hpm_qtd_s *qtd, uint32_t **bp,
                             void *arg);
...
struct hpm_ehci_s
{
  ...
  struct hpm_list_s *qhfree;  /* List of free Queue Head (QH) structures */
  struct hpm_list_s *qtdfree; /* List of free Queue Element Transfer Descriptor (qTD) */
  ...
  struct hpm_rhport_s rhport[HPM_EHCI_NRHPORT];
};
...
enum usbhost_trace1codes_e
{
  ...
  EHCI_TRACE1_QTDFOREACH_FAILED,    /* EHCI ERROR: hpm_qtd_foreach failed */
  ...
  EHCI_TRACE1_ADDBPL_FAILED,        /* EHCI ERROR: hpm_qtd_addbpl failed */
  ...
  EHCI_TRACE1_QHCREATE_FAILED,      /* EHCI ERROR: hpm_qh_create failed */
  EHCI_TRACE1_QTDSETUP_FAILED,      /* EHCI ERROR: hpm_qtd_setupphase failed */

  EHCI_TRACE1_QTDDATA_FAILED,       /* EHCI ERROR: hpm_qtd_dataphase failed */
  EHCI_TRACE1_QTDSTATUS_FAILED,     /* EHCI ERROR: hpm_qtd_statusphase failed */
  EHCI_TRACE1_TRANSFER_FAILED,      /* EHCI ERROR: Transfer failed */
  EHCI_TRACE1_QHFOREACH_FAILED,     /* EHCI ERROR: hpm_qh_foreach failed: */
  ...
  EHCI_TRACE1_RESET_FAILED,         /* EHCI ERROR: hpm_reset failed */
...
static uint16_t hpm_read16(const uint8_t *addr);
static uint32_t hpm_read32(const uint8_t *addr);
#if 0 /* Not used */
static void hpm_write16(uint16_t memval, uint8_t *addr);
static void hpm_write32(uint32_t memval, uint8_t *addr);
#endif

#ifdef CONFIG_ENDIAN_BIG
static uint16_t hpm_swap16(uint16_t value);
static uint32_t hpm_swap32(uint32_t value);
#else
#  define hpm_swap16(value) (value)
#  define hpm_swap32(value) (value)
#endif

#ifdef CONFIG_HPM_EHCI_REGDEBUG
static void hpm_printreg(volatile uint32_t *regaddr, uint32_t regval,
         bool iswrite);
static void hpm_checkreg(volatile uint32_t *regaddr, uint32_t regval,
         bool iswrite);
static uint32_t hpm_getreg(volatile uint32_t *regaddr);
static void hpm_putreg(uint32_t regval, volatile uint32_t *regaddr);
#else
static inline uint32_t hpm_getreg(volatile uint32_t *regaddr);
static inline void hpm_putreg(uint32_t regval, volatile uint32_t *regaddr);
#endif
...
static struct hpm_qh_s *hpm_qh_alloc(void);
static void hpm_qh_free(struct hpm_qh_s *qh);
static struct hpm_qtd_s *hpm_qtd_alloc(void);
static void hpm_qtd_free(struct hpm_qtd_s *qtd);

/* List Management **********************************************************/

static int hpm_qh_foreach(struct hpm_qh_s *qh, uint32_t **bp,
         foreach_qh_t handler, void *arg);
static int hpm_qtd_foreach(struct hpm_qh_s *qh, foreach_qtd_t handler,
         void *arg);
static int hpm_qtd_discard(struct hpm_qtd_s *qtd, uint32_t **bp,
         void *arg);
static int hpm_qh_discard(struct hpm_qh_s *qh);
...
#ifdef CONFIG_HPM_EHCI_REGDEBUG
static void hpm_qtd_print(struct hpm_qtd_s *qtd);
static void hpm_qh_print(struct hpm_qh_s *qh);
static int hpm_qtd_dump(struct hpm_qtd_s *qtd, uint32_t **bp, void *arg);
static int hpm_qh_dump(struct hpm_qh_s *qh, uint32_t **bp, void *arg);
#else
#  define hpm_qtd_print(qtd)
#  define hpm_qh_print(qh)
#  define hpm_qtd_dump(qtd, bp, arg)
#  define hpm_qh_dump(qh, bp, arg)
#endif

static inline uint8_t hpm_ehci_speed(uint8_t usbspeed);
static int hpm_ioc_setup(struct hpm_rhport_s *rhport,
         struct hpm_epinfo_s *epinfo);
static int hpm_ioc_wait(struct hpm_epinfo_s *epinfo);
static void hpm_qh_enqueue(struct hpm_qh_s *qhead,
         struct hpm_qh_s *qh);
static struct hpm_qh_s *hpm_qh_create(struct hpm_rhport_s *rhport,
         struct hpm_epinfo_s *epinfo);
static int hpm_qtd_addbpl(struct hpm_qtd_s *qtd, const void *buffer,
         size_t buflen);
static struct hpm_qtd_s *hpm_qtd_setupphase(
         struct hpm_epinfo_s *epinfo, const struct usb_ctrlreq_s *req);
static struct hpm_qtd_s *hpm_qtd_dataphase(struct hpm_epinfo_s *epinfo,
         void *buffer, int buflen, uint32_t tokenbits);
static struct hpm_qtd_s *hpm_qtd_statusphase(uint32_t tokenbits);
#ifndef CONFIG_USBHOST_INT_DISABLE
static int hpm_intr_setup(struct hpm_rhport_s *rhport,
         struct hpm_epinfo_s *epinfo, uint8_t *buffer, size_t buflen);
#endif
static ssize_t hpm_transfer_wait(struct hpm_epinfo_s *epinfo);
#ifdef CONFIG_USBHOST_ASYNCH
static inline int hpm_ioc_async_setup(struct hpm_rhport_s *rhport,
         struct hpm_epinfo_s *epinfo, usbhost_asynch_t callback,
         void *arg);
static void hpm_asynch_completion(struct hpm_epinfo_s *epinfo);
#endif

/* Interrupt Handling *******************************************************/

static int hpm_qtd_ioccheck(struct hpm_qtd_s *qtd, uint32_t **bp,
         void *arg);
static int hpm_qh_ioccheck(struct hpm_qh_s *qh, uint32_t **bp,
         void *arg);
#ifdef CONFIG_USBHOST_ASYNCH
static int hpm_qtd_cancel(struct hpm_qtd_s *qtd, uint32_t **bp,
         void *arg);
static int hpm_qh_cancel(struct hpm_qh_s *qh, uint32_t **bp, void *arg);
#endif
static inline void hpm_ioc_bottomhalf(void);
static inline void hpm_portsc_bottomhalf(void);
static inline void hpm_syserr_bottomhalf(void);
static inline void hpm_async_advance_bottomhalf(void);
static void hpm_ehci_bottomhalf(void *arg);
static int hpm_ehci_interrupt(int irq, void *context, void *arg);

/* USB Host Controller Operations *******************************************/

static int hpm_wait(struct usbhost_connection_s *conn,
                      struct usbhost_hubport_s **hport);
static int hpm_rh_enumerate(struct usbhost_connection_s *conn,
                              struct usbhost_hubport_s *hport);
static int hpm_enumerate(struct usbhost_connection_s *conn,
                           struct usbhost_hubport_s *hport);

static int hpm_ep0configure(struct usbhost_driver_s *drvr,
                              usbhost_ep_t ep0, uint8_t funcaddr,
                              uint8_t speed, uint16_t maxpacketsize);
static int hpm_epalloc(struct usbhost_driver_s *drvr,
                         const struct usbhost_epdesc_s *epdesc,
                         usbhost_ep_t *ep);
static int hpm_epfree(struct usbhost_driver_s *drvr, usbhost_ep_t ep);
static int hpm_alloc(struct usbhost_driver_s *drvr,
                       uint8_t **buffer, size_t *maxlen);
static int hpm_free(struct usbhost_driver_s *drvr,
                      uint8_t *buffer);
static int hpm_ioalloc(struct usbhost_driver_s *drvr,
                         uint8_t **buffer, size_t buflen);
static int hpm_iofree(struct usbhost_driver_s *drvr,
                        uint8_t *buffer);
static int hpm_ctrlin(struct usbhost_driver_s *drvr, usbhost_ep_t ep0,
                        const struct usb_ctrlreq_s *req,
                        uint8_t *buffer);
static int hpm_ctrlout(struct usbhost_driver_s *drvr, usbhost_ep_t ep0,
                         const struct usb_ctrlreq_s *req,
                         const uint8_t *buffer);
static ssize_t hpm_transfer(struct usbhost_driver_s *drvr,
                              usbhost_ep_t ep, uint8_t *buffer,
                              size_t buflen);
#ifdef CONFIG_USBHOST_ASYNCH
static int hpm_asynch(struct usbhost_driver_s *drvr, usbhost_ep_t ep,
                        uint8_t *buffer, size_t buflen,
                        usbhost_asynch_t callback, void *arg);
#endif
static int hpm_cancel(struct usbhost_driver_s *drvr, usbhost_ep_t ep);
#ifdef CONFIG_USBHOST_HUB
static int hpm_connect(struct usbhost_driver_s *drvr,
                         struct usbhost_hubport_s *hport,
                         bool connected);
#endif
static void hpm_disconnect(struct usbhost_driver_s *drvr,
                             struct usbhost_hubport_s *hport);

/* Initialization ***********************************************************/

static int hpm_reset(void);
...
ATTR_PLACE_AT_NONCACHEABLE struct hpm_qh_s g_asynchead aligned_data(32);

#ifndef CONFIG_USBHOST_INT_DISABLE
/* The head of the periodic queue */

ATTR_PLACE_AT_NONCACHEABLE struct hpm_qh_s g_intrhead aligned_data(32);

/* The frame list */

#ifdef CONFIG_HPM_EHCI_PREALLOCATE
ATTR_PLACE_AT_NONCACHEABLE uint32_t g_framelist[FRAME_LIST_SIZE] aligned_data(4096);
#else
...
ATTR_PLACE_AT_NONCACHEABLE struct hpm_qh_s g_qhpool[CONFIG_HPM_EHCI_NQHS]
                       aligned_data(32);

/* Queue Element Transfer Descriptor (qTD) pool */

ATTR_PLACE_AT_NONCACHEABLE struct hpm_qtd_s g_qtdpool[CONFIG_HPM_EHCI_NQTDS]
                        aligned_data(32);
...
static struct hpm_qh_s *g_qhpool;

/* Queue Element Transfer Descriptor (qTD) pool */

static struct hpm_qtd_s *g_qtdpool;
...
static const struct hpm_ehci_trace_s g_trace1[TRACE1_NSTRINGS] =
{
  ...
  TRENTRY(EHCI_TRACE1_ADDBPL_FAILED, TR_FMT1,
          "EHCI ERROR: hpm_qtd_addbpl failed: %d\n"),
...
static const struct hpm_ehci_trace_s g_trace2[TRACE2_NSTRINGS] =
{
...
static inline uint32_t hpm_read32(const uint8_t *addr)
{
#ifdef CONFIG_ENDIAN_BIG
  return (uint32_t)hpm_read16(&addr[0]) << 16 |
         (uint32_t)hpm_read16(&addr[2]);
#else
  return (uint32_t)hpm_read16(&addr[2]) << 16 |
         (uint32_t)hpm_read16(&addr[0]);
#endif
}
...
{
#ifdef CONFIG_ENDIAN_BIG
  hpm_write16(memval >> 16, &addr[0]);
  hpm_write16(memval & 0xffff, &addr[2]);
#else
  hpm_write16(memval & 0xffff, &addr[0]);
  hpm_write16(memval >> 16, &addr[2]);
#endif
}
#endif
...
static uint32_t hpm_swap32(uint32_t value)
{
  return (uint32_t)hpm_swap16((uint16_t)((value >> 16) & 0xffff)) |
         (uint32_t)hpm_swap16((uint16_t)(value & 0xffff)) << 16;
}
#endif
 ...
 *
 * Description:
 *   Print the contents of a HPM EHCI register
 *
 ****************************************************************************/
 ...
 *
 * Description:
 *   Check if it is time to output debug information for accesses to a HPM
 *   EHCI register
 *
 ****************************************************************************/
                           ...
                           bool iswrite)
{
  ...
  if (regaddr == prevaddr && regval == preval && prevwrite == iswrite)
  ...
  else
    {
          ...
          if (count == 1)
            {
              ...
              hpm_printreg(prevaddr, preval, prevwrite);
            }
          else
            {
              /* No.. More than one. */
      ...
      hpm_printreg(regaddr, regval, iswrite);
    }
}
#endif
 ...
 *
 * Description:
 *   Get the contents of an HPM register
 *
 ****************************************************************************/
...
static uint32_t hpm_getreg(volatile uint32_t *regaddr)
{
  ...
  hpm_checkreg(regaddr, regval, false);
  return regval;
}
#else
static inline uint32_t hpm_getreg(volatile uint32_t *regaddr)
 ...
 *
 * Description:
 *   Set the contents of an HPM register to a value
 *
 ****************************************************************************/
...
static void hpm_putreg(uint32_t regval, volatile uint32_t *regaddr)
{
  ...
  hpm_checkreg(regaddr, regval, true);
...
static struct hpm_qh_s *hpm_qh_alloc(void)
{
  ...
  qh = (struct hpm_qh_s *)g_ehci.qhfree;
  if (qh)
    {
      g_ehci.qhfree = ((struct hpm_list_s *)qh)->flink;
      memset(qh, 0, sizeof(struct hpm_qh_s));
    }
...
static void hpm_qh_free(struct hpm_qh_s *qh)
{
  struct hpm_list_s *entry = (struct hpm_list_s *)qh;
...
static struct hpm_qtd_s *hpm_qtd_alloc(void)
{
  struct hpm_qtd_s *qtd;

  /* Remove the qTD from the freelist */

  qtd = (struct hpm_qtd_s *)g_ehci.qtdfree;
  if (qtd)
    {
      g_ehci.qtdfree = ((struct hpm_list_s *)qtd)->flink;
      memset(qtd, 0, sizeof(struct hpm_qtd_s));
    }
...
static void hpm_qtd_free(struct hpm_qtd_s *qtd)
{
  struct hpm_list_s *entry = (struct hpm_list_s *)qtd;
  ...
  while (qh)
    {
      ...
      else if (hpm_virtramaddr(physaddr & QH_HLP_MASK) ==
       ...
       */
        ...
        {
          physaddr = hpm_swap32(qh->hw.hlp) & QH_HLP_MASK;
          next     = (struct hpm_qh_s *)hpm_virtramaddr(physaddr);
        }
                             ...
                             void *arg)
{
  ...
  physaddr = hpm_swap32(*bp); /* Physical address of first qTD in CPU order */
  ...
  qtd  = (struct hpm_qtd_s *)hpm_virtramaddr(physaddr);
  next = NULL;
  ...
  while (qtd)
    {
      ...
      if ((hpm_swap32(qtd->hw.nqp) & QTD_NQP_T) != 0)
        {
          /* Set the next pointer to NULL.  This will terminate the loop. */
        ...
        {
          physaddr = hpm_swap32(qtd->hw.nqp) & QTD_NQP_NTEP_MASK;
          next     = (struct hpm_qtd_s *)hpm_virtramaddr(physaddr);
        }
...
/****************************************************************************
 * Name: hpm_qtd_discard
 *
 * Description:
 *   This is a hpm_qtd_foreach callback.  It simply unlinks the QTD,
...
static int hpm_qh_discard(struct hpm_qh_s *qh)
{
  ...
  ret = hpm_qtd_foreach(qh, hpm_qtd_discard, NULL);
  if (ret < 0)
    {
...
/****************************************************************************
 * Name: hpm_qtd_invalidate
 *
 * Description:
 *   This is a callback from hpm_qtd_foreach.  It simply invalidates D-
...
static int hpm_qh_invalidate(struct hpm_qh_s *qh)
{
  ...
  return hpm_qtd_foreach(qh, hpm_qtd_invalidate, NULL);
}
#endif
 ...
 *
 * Description:
 *   This is a callback from hpm_qtd_foreach.  It simply flushes D-cache
 *   for address range of the qTD entry.
 *
 ****************************************************************************/

static int hpm_qtd_flush(struct hpm_qtd_s *qtd, uint32_t **bp, void *arg)
{
 ...
 *
 * Description:
 *   This is a hpm_qtd_foreach callout function.  It dumps the context of
 *   one qTD
 *
 ****************************************************************************/
 ...
 *
 * Description:
 *   This is a hpm_qh_foreach call-out function.  It dumps a QH structure
...
{
  hpm_qh_print(qh);
  return hpm_qtd_foreach(qh, hpm_qtd_dump, NULL);
}
#endif
...
static void hpm_qh_enqueue(struct hpm_qh_s *qhead, struct hpm_qh_s *qh)
{
  uintptr_t physaddr;
                                          ...
                                          struct hpm_epinfo_s *epinfo)
{
  ...
  regval = ((uint32_t)epinfo->devaddr << QH_EPCHAR_DEVADDR_SHIFT) |
           ...
           ((uint32_t)hpm_ehci_speed(epinfo->speed) <<
            QH_EPCHAR_EPS_SHIFT) |
           QH_EPCHAR_DTC |
  ...
  qh->hw.epchar = hpm_swap32(regval);
...
#warning REVISIT
  ...
  qh->hw.epcaps = hpm_swap32(regval);

  /* Mark this as the end of this list.  This will be overwritten if/when the
   * next qTD is added to the queue.
   */

  qh->hw.hlp         = hpm_swap32(QH_HLP_T);
  qh->hw.overlay.nqp = hpm_swap32(QH_NQP_T);
  qh->hw.overlay.alt = hpm_swap32(QH_AQP_T);
  return qh;
}
                            ...
                            size_t buflen)
{
  ...
  physaddr = (uint32_t)hpm_physramaddr((uintptr_t)buffer);
  ...
  for (ndx = 0; ndx < 5; ndx++)
    {
      ...
      qtd->hw.bpl[ndx] = hpm_swap32(physaddr);
...
static struct hpm_qtd_s *
  hpm_qtd_setupphase(struct hpm_epinfo_s *epinfo,
                       const struct usb_ctrlreq_s *req)
{
  struct hpm_qtd_s *qtd;
  uint32_t regval;
  ...
  qtd->hw.nqp = hpm_swap32(QTD_NQP_T);
  qtd->hw.alt = hpm_swap32(QTD_AQP_T);
  ...
  qtd->hw.token = hpm_swap32(regval);

  /* Add the buffer data */

  ret = hpm_qtd_addbpl(qtd, req, USB_SIZEOF_CTRLREQ);
  if (ret < 0)
    {
                                               ...
                                               uint32_t tokenbits)
{
  ...
  ret = hpm_qtd_addbpl(qtd, buffer, buflen);
  if (ret < 0)
    {
      usbhost_trace1(EHCI_TRACE1_ADDBPL_FAILED, -ret);
      hpm_qtd_free(qtd);
...
static int hpm_async_setup(struct hpm_rhport_s *rhport,
                             struct hpm_epinfo_s *epinfo,
                             const struct usb_ctrlreq_s *req,
                             uint8_t *buffer, size_t buflen)
{
  ...
  qh = hpm_qh_create(rhport, epinfo);
  if (qh == NULL)
    {
  ...
  if (req != NULL)
    {
      ...
      qtd = hpm_qtd_setupphase(epinfo, req);
      if (qtd == NULL)
        {
      ...
      physaddr = hpm_physramaddr((uintptr_t)qtd);
      *flink = hpm_swap32(physaddr);
  ...
  if (buffer != NULL && buflen > 0)
    {
      ...
      qtd = hpm_qtd_dataphase(epinfo, buffer, buflen, tokenbits);
      if (qtd == NULL)
        {
  ...
  if (req != NULL)
    {
      ...
      qtd = hpm_qtd_statusphase(tokenbits);
      if (qtd == NULL)
        {
      ...
      *flink = hpm_swap32(physaddr);
      ...
      if (alt)
        {
          *alt = hpm_swap32(physaddr);
        }
    }

  /* Add the new QH to the head of the asynchronous queue list */

  hpm_qh_enqueue(&g_asynchead, qh);
  return OK;
...
/****************************************************************************
 * Name: hpm_qtd_ioccheck
 *
 * Description:
 *   This function is a hpm_qtd_foreach() callback function.  It services
...
static int hpm_qtd_ioccheck(struct hpm_qtd_s *qtd, uint32_t **bp,
                              void *arg)
{
  struct hpm_epinfo_s *epinfo = (struct hpm_epinfo_s *)arg;
  ...
  **bp = qtd->hw.nqp;
  ...
  epinfo->xfrd -= (hpm_swap32(qtd->hw.token) & QTD_TOKEN_NBYTES_MASK) >>
...
/****************************************************************************
 * Name: hpm_qh_ioccheck
 *
 * Description:
 *   This function is a hpm_qh_foreach() callback function.  It services
...
static int hpm_qh_ioccheck(struct hpm_qh_s *qh, uint32_t **bp, void *arg)
{
  struct hpm_epinfo_s *epinfo;
  uint32_t token;
  int ret;
  ...
  token = hpm_swap32(qh->hw.overlay.token);
  ...
  ret = hpm_qtd_foreach(qh, hpm_qtd_ioccheck, (void *)qh->epinfo);
  if (ret < 0)
    {
      usbhost_trace1(EHCI_TRACE1_QTDFOREACH_FAILED, -ret);
    }
  ...
  if ((hpm_swap32(qh->fqp) & QTD_NQP_T) != 0)
    {
 ...
 *
 * Description:
 *   This function is a hpm_qtd_foreach() callback function.  It removes
 ...
 *
 * Description:
 *   This function is a hpm_qh_foreach() callback function.  It cancels
...
static int hpm_qh_cancel(struct hpm_qh_s *qh, uint32_t **bp, void *arg)
{
  ...
  regval = hpm_getreg(&HCOR->usbcmd);
  hpm_putreg(regval & ~(EHCI_USBCMD_ASEN | EHCI_USBCMD_PSEN),
               &HCOR->usbcmd);
  ...
  **bp = qh->hw.hlp;
  ...
  ret = hpm_qtd_foreach(qh, hpm_qtd_cancel, NULL);
  if (ret < 0)
    {
      usbhost_trace1(EHCI_TRACE1_QTDFOREACH_FAILED, -ret);
    }
...
static inline void hpm_ioc_bottomhalf(void)
{
  ...
  qh = (struct hpm_qh_s *)
       hpm_virtramaddr(hpm_swap32(*bp) & QH_HLP_MASK);
  ...
  if (qh && qh != &g_asynchead)
    {
      ...
      ret = hpm_qh_foreach(qh, &bp, hpm_qh_ioccheck, NULL);
      if (ret < 0)
        {
...
static inline void hpm_portsc_bottomhalf(void)
{
  ...
  for (rhpndx = 0; rhpndx < HPM_EHCI_NRHPORT; rhpndx++)
    {
      rhport = &g_ehci.rhport[rhpndx];
      portsc = hpm_getreg(&HCOR->portsc[rhpndx]);
      ...
      hpm_putreg(portsc, &HCOR->portsc[rhpndx]);
    }
}

/****************************************************************************
 * Name: hpm_syserr_bottomhalf
 *
 * Description:
...
/****************************************************************************
 * Name: hpm_async_advance_bottomhalf
 *
 * Description:
...
static void hpm_ehci_bottomhalf(void *arg)
{
  ...
  hpm_putreg(EHCI_HANDLED_INTS, &HCOR->usbintr);
}

/****************************************************************************
 * Name: hpm_ehci_interrupt
 *
 * Description:
...
static int hpm_ehci_interrupt(int irq, void *context, void *arg)
{
  ...
  usbsts = hpm_getreg(&HCOR->usbsts);
  regval = hpm_getreg(&HCOR->usbintr);
  ...
  if (pending != 0)
    {
      ...
      DEBUGVERIFY(work_queue(HPWORK, &g_ehci.work, hpm_ehci_bottomhalf,
                            (void *)pending, 0));
                              ...
                              struct usbhost_hubport_s *hport)
{
  ...
  DEBUGASSERT(rhpndx >= 0 && rhpndx < HPM_EHCI_NRHPORT);
  rhport = &g_ehci.rhport[rhpndx];
  ...
  regval = hpm_getreg(&HCOR->portsc[rhpndx]);
  ...
  DEBUGASSERT((hpm_getreg(&HCOR->usbsts) & EHCI_USBSTS_HALTED) == 0);
  ...
  regval  = hpm_getreg(regaddr);
  if (hport->speed == USB_SPEED_LOW)
    {
      regval |= USB_PORTSC1_STS_MASK;
    }
  else
    {
      regval &= ~USB_PORTSC1_STS_MASK;
  ...
  while ((hpm_getreg(regaddr) & EHCI_PORTSC_RESET) != 0);
  ...
  if (USB_PORTSC1_PSPD_GET(regval) == 0x02)
    {
      /* High speed device */

      hport->speed = USB_SPEED_HIGH;
    }
  else if (USB_PORTSC1_PSPD_GET(regval) == 0x00)
    {
...
#endif
    {
      ret = hpm_rh_enumerate(conn, hport);
      if (ret < 0)
        {
          return ret;
        }
...
static int hpm_ep0configure(struct usbhost_driver_s *drvr,
                              ...
                              uint8_t speed, uint16_t maxpacketsize)
{
  struct hpm_epinfo_s *epinfo = (struct hpm_epinfo_s *)ep0;
  int ret;
                         ...
                         usbhost_ep_t *ep)
{
  ...
  epinfo = (struct hpm_epinfo_s *)
    kmm_zalloc(sizeof(struct hpm_epinfo_s));
  if (!epinfo)
    {
   ...
   * inside of struct hpm_epinfo_s and just memcpy() here.
   */
                       ...
                       uint8_t **buffer, size_t *maxlen)
{
  ...
  *buffer = (uint8_t *)kmm_memalign(HPM_L1C_CACHELINE_SIZE,
                                    HPM_EHCI_BUFSIZE);
  if (*buffer)
    {
      *maxlen = HPM_EHCI_BUFSIZE;
      ret = OK;
    }
...
static int hpm_free(struct usbhost_driver_s *drvr, uint8_t *buffer)
{
                         ...
                         uint8_t **buffer, size_t buflen)
{
  ...
  *buffer = (uint8_t *)kumm_memalign(HPM_L1C_CACHELINE_SIZE, buflen);
...
/****************************************************************************
 * Name: hpm_ctrlin and hpm_ctrlout
 *
 * Description:
...
static int hpm_ctrlin(struct usbhost_driver_s *drvr, usbhost_ep_t ep0,
                        ...
                        uint8_t *buffer)
{
  struct hpm_rhport_s *rhport = (struct hpm_rhport_s *)drvr;
  struct hpm_epinfo_s *ep0info = (struct hpm_epinfo_s *)ep0;
  uint16_t len;
  ssize_t nbytes;
  int ret;
  ...
  ret = hpm_ioc_setup(rhport, ep0info);
  if (ret != OK)
    {
  ...
  ret = hpm_async_setup(rhport, ep0info, req, buffer, len);
  if (ret < 0)
    {
      uerr("ERROR: hpm_async_setup failed: %d\n", ret);
...
static int hpm_ctrlout(struct usbhost_driver_s *drvr, usbhost_ep_t ep0,
                         ...
                         const uint8_t *buffer)
{
  /* hpm_ctrlin can handle both directions.  We just need to work around
   * the differences in the function signatures.
   */

  return hpm_ctrlin(drvr, ep0, req, (uint8_t *)buffer);
}
                              ...
                              size_t buflen)
{
  ...
  ret = hpm_ioc_setup(rhport, epinfo);
  if (ret != OK)
    {
      usbhost_trace1(EHCI_TRACE1_DEVDISCONNECTED, -ret);
      goto errout_with_lock;
  ...
  switch (epinfo->xfrtype)
    {
      case USB_EP_ATTR_XFER_BULK:
        ret = hpm_async_setup(rhport, epinfo, NULL, buffer, buflen);
        break;

#ifndef CONFIG_USBHOST_INT_DISABLE
      case USB_EP_ATTR_XFER_INT:
        ret = hpm_intr_setup(rhport, epinfo, buffer, buflen);
        break;
#endif
                        ...
                        usbhost_asynch_t callback, void *arg)
{
  ...
  ret = hpm_ioc_async_setup(rhport, epinfo, callback, arg);
  if (ret != OK)
    {
      usbhost_trace1(EHCI_TRACE1_DEVDISCONNECTED, -ret);
      goto errout_with_lock;
...
# warning "Isochronous endpoint support not emplemented"
  ...
  ret = hpm_qh_foreach(qh, &bp, hpm_qh_cancel, epinfo);
  if (ret < 0)
    {
      usbhost_trace1(EHCI_TRACE1_QTDFOREACH_FAILED, -ret);
    }
 ...
 *            Since the HPM has only a downstream port, zero is
...
static void hpm_usbhost_vbusdrive(int rhport, bool enable)
{
  uint32_t regval;

  uinfo("RHPort%d: enable=%d\n", rhport + 1, enable);

  /* The HPM has only a single root hub port */
...
struct usbhost_connection_s *hpm_ehci_initialize(int controller)
{
  struct usbhost_hubport_s *hport;
...
#  endif
  ...
  DEBUGASSERT((sizeof(struct hpm_qh_s) & 0x1f) == 0);
  DEBUGASSERT((sizeof(struct hpm_qtd_s) & 0x1f) == 0);
...
#  endif /* CONFIG_USBHOST_INT_DISABLE */
  ...
  HCOR = (struct ehci_hcor_s *)(&usb_instance->USBCMD);
  ...
  for (i = 0; i < HPM_EHCI_NRHPORT; i++)
    {
      struct hpm_rhport_s *rhport = &g_ehci.rhport[i];

      /* Initialize the device operations */

      rhport->drvr.ep0configure = hpm_ep0configure;
      rhport->drvr.epalloc = hpm_epalloc;
      rhport->drvr.epfree = hpm_epfree;
      rhport->drvr.alloc = hpm_alloc;
      rhport->drvr.free = hpm_free;
      rhport->drvr.ioalloc = hpm_ioalloc;
      rhport->drvr.iofree = hpm_iofree;
      rhport->drvr.ctrlin = hpm_ctrlin;
      rhport->drvr.ctrlout = hpm_ctrlout;
      rhport->drvr.transfer = hpm_transfer;
#  ifdef CONFIG_USBHOST_ASYNCH
      rhport->drvr.asynch = hpm_asynch;
#  endif
      rhport->drvr.cancel = hpm_cancel;
#  ifdef CONFIG_USBHOST_HUB
...
#  ifndef CONFIG_HPM_EHCI_PREALLOCATE
  ...
  g_qhpool =
    (struct hpm_qh_s *)kmm_memalign(32,
                                      CONFIG_HPM_EHCI_NQHS *
                                      sizeof(struct hpm_qh_s));
  if (!g_qhpool)
    {
...
#  endif
  ...
  for (i = 0; i < CONFIG_HPM_EHCI_NQHS; i++)
    {
      /* Put the QH structure in a free list */

      hpm_qh_free(&g_qhpool[i]);
    }
...
#  ifndef CONFIG_HPM_EHCI_PREALLOCATE
  ...
  g_qtdpool =
    (struct hpm_qtd_s *)kmm_memalign(32,
                                       CONFIG_HPM_EHCI_NQTDS *
                                       sizeof(struct hpm_qtd_s));
  if (!g_qtdpool)
    {
...
#  if !defined(CONFIG_HPM_EHCI_PREALLOCATE) && !defined(CONFIG_USBHOST_INT_DISABLE)
...
#  endif
  ...
  for (i = 0; i < CONFIG_HPM_EHCI_NQTDS; i++)
    {
      /* Put the TD in a free list */

      hpm_qtd_free(&g_qtdpool[i]);
    }

  /* Reset the controller from the OTG peripheral */

  putreg32(EHCI_USBCMD_HCRESET, &HCOR->usbcmd);
  while ((getreg32(&HCOR->usbcmd) & EHCI_USBCMD_HCRESET) != 0);
...
#  ifdef CONFIG_HPM_EHCI_SDIS
  putreg32(USB_USBMODE_CM_SET(3) | USB_USBMODE_SDIS_MASK,
           &usb_instance->USBMODE);
#  else
  putreg32(USB_USBMODE_CM_SET(3),
           &usb_instance->USBMODE);
#  endif
  ...
  /* Re-program the USB host controller.  As implemented, hpm_reset()
...
#  endif
  ...
  /* Set the endian */
  usb_instance->USBMODE &= ~USB_USBMODE_ES_MASK;

  /* Set parallel interface signal */
  usb_instance->PORTSC1 &= ~USB_PORTSC1_STS_MASK;

  /* Set parallel transceiver width */
  usb_instance->PORTSC1 &= ~USB_PORTSC1_PTW_MASK;

  /* Not use interrupt threshold. */
  usb_instance->USBCMD &= ~USB_USBCMD_ITC_MASK;
  ...
  hpm_putreg(EHCI_INT_ALLINTS, &HCOR->usbsts);
  ...
  memset(&g_asynchead, 0, sizeof(struct hpm_qh_s));
  physaddr = hpm_physramaddr((uintptr_t)&g_asynchead);
  g_asynchead.hw.hlp = hpm_swap32(physaddr | QH_HLP_TYP_QH);
  g_asynchead.hw.epchar = hpm_swap32(QH_EPCHAR_H | QH_EPCHAR_EPS_FULL);
  g_asynchead.hw.overlay.nqp = hpm_swap32(QH_NQP_T);
  g_asynchead.hw.overlay.alt = hpm_swap32(QH_NQP_T);
  g_asynchead.hw.overlay.token = hpm_swap32(QH_TOKEN_HALTED);
...
#  ifndef CONFIG_USBHOST_INT_DISABLE
  ...
  memset(&g_intrhead, 0, sizeof(struct hpm_qh_s));
  g_intrhead.hw.hlp = hpm_swap32(QH_HLP_T);
  g_intrhead.hw.overlay.nqp = hpm_swap32(QH_NQP_T);
  g_intrhead.hw.overlay.alt = hpm_swap32(QH_NQP_T);
  g_intrhead.hw.overlay.token = hpm_swap32(QH_TOKEN_HALTED);
  g_intrhead.hw.epcaps = hpm_swap32(QH_EPCAPS_SSMASK(1));

  /* Attach the periodic QH to Period Frame List */

  physaddr = hpm_physramaddr((uintptr_t)&g_intrhead);
  for (i = 0; i < FRAME_LIST_SIZE; i++)
    {
      g_framelist[i] = hpm_swap32(physaddr) | PFL_TYP_QH;
    }

  /* Set the Periodic Frame List Base Address. */

  physaddr = hpm_physramaddr((uintptr_t) g_framelist);
  hpm_putreg(hpm_swap32(physaddr), &HCOR->periodiclistbase);
#  endif
...
#  endif
  ...
  regval = hpm_getreg(&HCOR->configflag);
  regval |= EHCI_CONFIGFLAG;
  hpm_putreg(regval, &HCOR->configflag);
  ...
  if (ret < 0)
    {
      usbhost_trace1(EHCI_TRACE1_RUN_FAILED, hpm_getreg(&HCOR->usbsts));
      return NULL;
    }

  /* Interrupt Configuration ************************************************/

  ret = irq_attach(irq_num, hpm_ehci_interrupt, NULL);
  if (ret != 0)
    {
      usbhost_trace1(EHCI_TRACE1_IRQATTACH_FAILED, irq_num);
      return NULL;
    }
  ...
  for (i = 0; i < HPM_EHCI_NRHPORT; i++)
    {
      g_ehci.rhport[i].connected =
        ((hpm_getreg(&HCOR->portsc[i]) & EHCI_PORTSC_CCS) != 0);
    }
