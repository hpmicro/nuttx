/****************************************************************************
 * boards/risc-v/hpmicro/hpm6300evk-sdk/src/hpm6300_gpio.c
 *
...
struct hpm6300_gpio_dev_s
{
  struct gpio_dev_s gpio;
  uint32_t           id;
};

struct hpm6300_gpint_dev_s
{
  struct hpm6300_gpio_dev_s hpm6300gpio;
...
static struct hpm6300_gpio_dev_s g_gpin[BOARD_NGPIOIN];
#endif
...
static struct hpm6300_gpio_dev_s g_gpout[BOARD_NGPIOOUT];
#endif
...
static struct hpm6300_gpint_dev_s g_gpint[BOARD_NGPIOINT];
#endif
...
static int hpm6300_gpio_interrupt(int irq, void *context, void *arg)
{
  struct hpm6300_gpint_dev_s *hpm6300xgpint =
    (struct hpm6300_gpint_dev_s *)arg;

  uint32_t gpio_pin;

  DEBUGASSERT(hpm6300xgpint != NULL && hpm6300xgpint->callback != NULL);
  ...
  hpm6300xgpint->callback(&hpm6300xgpint->hpm6300gpio.gpio,
                        gpio_pin);
...
static int gpio_setpintype(struct gpio_dev_s *dev,
                           enum gpio_pintype_e    gp_pintype)
{
  struct hpm6300_gpint_dev_s *hpm6300gpint =
    (struct hpm6300_gpint_dev_s *)dev;
  uint32_t gpio_pin;
  uint8_t pintype = hpm6300gpint->hpm6300gpio.gpio.gp_pintype;
  int ret = 0;

  DEBUGASSERT(hpm6300gpint != NULL);
  gpioinfo("setpintype...\n");
  ...
  gpio_pin = g_gpiointinputs[hpm6300gpint->hpm6300gpio.id];
...
static int gpin_read(struct gpio_dev_s *dev, bool *value)
{
  struct hpm6300_gpio_dev_s *hpm63xxgpio =
    (struct hpm6300_gpio_dev_s *)dev;

  DEBUGASSERT(hpm63xxgpio != NULL && value != NULL);
  gpioinfo("Reading...\n");
  *value = hpm_gpioread(BOARD_APP_GPIO_CTRL, g_gpioinputs[hpm63xxgpio->id], GPIO_INPUT_MODE);
...
static int gpout_read(struct gpio_dev_s *dev, bool *value)
{
  ...
  DEBUGASSERT(hpm63xxgpio != NULL && value != NULL);
  DEBUGASSERT(hpm63xxgpio->id < BOARD_NGPIOOUT);
  gpioinfo("Reading...\n");

  uint32_t gpio_pin = g_gpiooutputs[hpm63xxgpio->id];
...
static int gpout_write(struct gpio_dev_s *dev, bool value)
{
  ...
  DEBUGASSERT(hpm63xxgpio != NULL);
  DEBUGASSERT(hpm63xxgpio->id < BOARD_NGPIOOUT);
  gpioinfo("Writing %d\n", (int)value);

  hpm_gpiowrite(BOARD_APP_GPIO_CTRL, g_gpiooutputs[hpm63xxgpio->id], value);
...
static int gpint_read(struct gpio_dev_s *dev, bool *value)
{
  ...
  DEBUGASSERT(hpm6300xgpint != NULL && value != NULL);
  DEBUGASSERT(hpm6300xgpint->hpm6300gpio.id < BOARD_NGPIOINT);
  gpioinfo("Reading int pin...\n");

  *value = hpm_gpioread(BOARD_APP_GPIO_CTRL, g_gpiointinputs[hpm6300xgpint->hpm6300gpio.id], GPIO_INPUT_MODE);
...
static int gpint_attach(struct gpio_dev_s *dev, pin_interrupt_t callback)
{
  ...
  hpm6300xgpint->callback = callback;

  irq_attach(gpio_get_irq_index(BOARD_APP_GPIO_CTRL, gpio_pin), hpm6300_gpio_interrupt, (void *)dev);
...
int hpm6300_gpio_initialize(void)
{
  int i;
  int pincount = 0;
  uint32_t gpio_pin = 0;
  ...
  for (i = 0; i < BOARD_NGPIOINT; i++)
    {
      ...
      g_gpint[i].hpm6300gpio .gpio.gp_pintype = GPIO_INTERRUPT_PIN;
      g_gpint[i].hpm6300gpio.gpio.gp_ops     = &gpint_ops;
      g_gpint[i].hpm6300gpio.id              = i;
      gpio_pin_register(&g_gpint[i].hpm6300gpio.gpio, pincount);
