/****************************************************************************
 * boards/risc-v/hpmicro/hpm6300evk/src/hpm6300_bringup.c
 *
...
#  include "hpm6300_i2c.h"
#endif

#ifdef CONFIG_HPM_CAN_DRV
#  include"hpm6300_can.h"
#endif
...
int hpm6300_bringup(void)
{
  ...
  ret = hpm6300evk_i2cdev_initialize(0);
  if (ret < 0)
    {
      syslog(LOG_ERR, "ERROR: i2c0 hpm6300evk_i2cdev_initialize failed: %d\n", ret);
    }
#endif
...
#  ifdef CONFIG_HPM_PWM2
  ...
  if (pwm == NULL)
    {
      syslog(LOG_DEBUG, "ERROR: hpm6300_pwminitialize failed\n");
    }
  else
    {
...
#if defined(CONFIG_HPM_USBOTG) && defined(CONFIG_USBHOST)
  ret = hpm6300_usbhost_initialize();
  if (ret != OK)
    {
