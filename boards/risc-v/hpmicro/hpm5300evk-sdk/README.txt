...
5. Configure and build NuttX
  ...
  $ cd ../nuttx
  $ make distclean
  $ ./tools/configure.sh -l hpm5300evk-sdk:nsh
  $ make V=1
...
6. Flash the nuttx with openocd and run
  ...
  The openocd cfg files in the path: sdk_env/hpm_sdk/boards/openocd. If you use on-board debugger, in windows shell, start openocd command as follows.
    $ openocd -f probes/ft2232.cfg -f soc/hpm5300.cfg -f boards/hpm5300evk.cfg
...
7. HPMicro Source
  ...
  [3] [path to nuttxspace]/nuttx/boards/risc-v/hpmicro/hpm5300evk-sdk
