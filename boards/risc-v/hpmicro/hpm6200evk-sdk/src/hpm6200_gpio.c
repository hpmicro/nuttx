/****************************************************************************
 * boards/risc-v/hpmicro/hpm6200evk-sdk/src/hpm6200_gpio.c
 *
...
struct hpm6200_gpio_dev_s
{
  struct gpio_dev_s gpio;
  uint32_t           id;
};

struct hpm6200_gpint_dev_s
{
  struct hpm6200_gpio_dev_s hpm6200gpio;
...
static struct hpm6200_gpio_dev_s g_gpout[BOARD_NGPIOOUT];
#endif
...
static struct hpm6200_gpint_dev_s g_gpint[BOARD_NGPIOINT];
#endif
...
static int hpm6200_gpio_interrupt(int irq, void *context, void *arg)
{
  struct hpm6200_gpint_dev_s *hpm6200xgpint =
    (struct hpm6200_gpint_dev_s *)arg;

  uint32_t gpio_pin;

  DEBUGASSERT(hpm6200xgpint != NULL && hpm6200xgpint->callback != NULL);
  ...
  hpm6200xgpint->callback(&hpm6200xgpint->hpm6200gpio.gpio,
                        gpio_pin);
...
static int gpio_setpintype(struct gpio_dev_s *dev,
                           enum gpio_pintype_e    gp_pintype)
{
  struct hpm6200_gpint_dev_s *hpm6200gpint =
    (struct hpm6200_gpint_dev_s *)dev;
  uint32_t gpio_pin;
  uint8_t pintype = hpm6200gpint->hpm6200gpio.gpio.gp_pintype;
  int ret = 0;

  DEBUGASSERT(hpm6200gpint != NULL);
  gpioinfo("setpintype...\n");
  ...
  gpio_pin = g_gpiointinputs[hpm6200gpint->hpm6200gpio.id];
...
static int gpout_read(struct gpio_dev_s *dev, bool *value)
{
  struct hpm6200_gpio_dev_s *hpm6200xgpio =
    (struct hpm6200_gpio_dev_s *)dev;

  DEBUGASSERT(hpm6200xgpio != NULL && value != NULL);
  DEBUGASSERT(hpm6200xgpio->id < BOARD_NGPIOOUT);
  gpioinfo("Reading...\n");
...
static int gpout_write(struct gpio_dev_s *dev, bool value)
{
  ...
  DEBUGASSERT(hpm6200xgpio != NULL);
  DEBUGASSERT(hpm6200xgpio->id < BOARD_NGPIOOUT);
  gpioinfo("Writing %d\n", (int)value);

  hpm_gpiowrite(BOARD_APP_GPIO_CTRL, g_gpiooutputs[hpm6200xgpio->id], value);
...
static int gpint_read(struct gpio_dev_s *dev, bool *value)
{
  ...
  DEBUGASSERT(hpm6200xgpint != NULL && value != NULL);
  DEBUGASSERT(hpm6200xgpint->hpm6200gpio.id < BOARD_NGPIOINT);
  gpioinfo("Reading int pin...\n");

  *value = hpm_gpioread(BOARD_APP_GPIO_CTRL, g_gpiointinputs[hpm6200xgpint->hpm6200gpio.id], GPIO_INPUT_MODE);
...
static int gpint_attach(struct gpio_dev_s *dev, pin_interrupt_t callback)
{
  ...
  hpm6200xgpint->callback = callback;

  irq_attach(gpio_get_irq_index(BOARD_APP_GPIO_CTRL, gpio_pin), hpm6200_gpio_interrupt, (void *)dev);
...
int hpm6200_gpio_initialize(void)
{
  int i;
  int pincount = 0;
  uint32_t gpio_pin = 0;
  ...
  for (i = 0; i < BOARD_NGPIOINT; i++)
    {
      ...
      g_gpint[i].hpm6200gpio .gpio.gp_pintype = GPIO_INTERRUPT_PIN;
      g_gpint[i].hpm6200gpio.gpio.gp_ops     = &gpint_ops;
      g_gpint[i].hpm6200gpio.id              = i;
      gpio_pin_register(&g_gpint[i].hpm6200gpio.gpio, pincount);
