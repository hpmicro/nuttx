/****************************************************************************
 * boards/risc-v/hpmicro/hpm6200evk/src/hpm6200_bringup.c
 *
...
#  include "hpm6200_i2c.h"
#endif

#ifdef CONFIG_HPM_CAN_DRV
#  include"hpm6200_can.h"
#endif
...
int hpm6200_bringup(void)
{
  ...
  ret = hpm6200evk_i2cdev_initialize(0);
  if (ret < 0)
    {
      syslog(LOG_ERR, "ERROR: i2c0 hpm6200evk_i2cdev_initialize failed: %d\n", ret);
    }
#endif
...
#  ifdef CONFIG_HPM_PWM2
  ...
  if (pwm == NULL)
    {
      syslog(LOG_DEBUG, "ERROR: hpm6200_pwminitialize failed\n");
    }
  else
    {
...
#if defined(CONFIG_HPM_USBOTG) && defined(CONFIG_USBHOST)
  ret = hpm6200_usbhost_initialize();
  if (ret != OK)
    {
